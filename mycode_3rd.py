...
while counter <= 10: # 반복문으로 counter의 값이 10이 될때까지 10번 오면 종료한다.  
    grade = int(input("성적을 입력하시오.: ")) # 변수 grade는 정수형 변현문 1nt를 이용하여 입력받는다.  
    total = grade + total # 변수 grade의 값과 total의 값을 계산하여 다시 total에 선언(10번 증가) 
    counter = counter + 1 # 변수 cointer의 값을 한번 실행시킬때마다 1씩 증가 
